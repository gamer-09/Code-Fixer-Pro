    ...
    <View style={[styles.container, { backgroundColor: colors.void }]}>
      ...
      >
        ...
        {(() => {
          ...
          const bestCoin = loaded.reduce((best, c) =>
            (data[c.sym]?.regularMarketChangePercent ?? -Infinity) > (data[best.sym]?.regularMarketChangePercent ?? -Infinity) ? c : best
          );
          const worstCoin = loaded.reduce((worst, c) =>
            (data[c.sym]?.regularMarketChangePercent ?? Infinity) < (data[worst.sym]?.regularMarketChangePercent ?? Infinity) ? c : worst
          );
          return (
            <>
              ...
              <View style={[styles.contextCard, { backgroundColor: colors.card, borderColor: colors.rim }]}>
                <View style={styles.summaryRow}>
                  <View style={styles.summaryItem}>
                    ...
                    <Text style={[styles.summaryLbl, { color: colors.t4 }]}>Gaining</Text>
                  </View>
                  <View style={[styles.summaryDivider, { backgroundColor: colors.rim }]} />
                  <View style={styles.summaryItem}>
                    <Text style={[styles.summaryNum, { color: colors.loss }]}>{dn}</Text>
                    <Text style={[styles.summaryLbl, { color: colors.t4 }]}>Declining</Text>
                  </View>
                  <View style={[styles.summaryDivider, { backgroundColor: colors.rim }]} />
                  <View style={styles.summaryItem}>
                ...
                <View style={[styles.leaderRow, { borderTopColor: colors.rim }]}>
                  <Text style={[styles.leaderText, { color: colors.t3 }]}>
                    Best: <Text style={{ color: colors.gain, fontFamily: 'Inter_600SemiBold' }}>{bestCoin.label}</Text>
                    {'  '}
                    {fmtChg(data[bestCoin.sym]?.regularMarketChangePercent)}
                  </Text>
                  <Text style={[styles.leaderText, { color: colors.t3 }]}>
                    Worst: <Text style={{ color: colors.loss, fontFamily: 'Inter_600SemiBold' }}>{worstCoin.label}</Text>
                    {'  '}
                    {fmtChg(data[worstCoin.sym]?.regularMarketChangePercent)}
                  </Text>
                </View>
              </View>
            </>
...
const styles = StyleSheet.create({
  ...
  contextCard: { borderRadius: 8, borderWidth: 1, overflow: 'hidden', marginBottom: 10 },
  summaryRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 8 },
  summaryItem: { flex: 1, alignItems: 'center' },
  summaryNum: { fontSize: 18, fontFamily: 'Inter_700Bold', marginBottom: 2 },
  summaryLbl: { fontSize: 8, fontFamily: 'Inter_500Medium', textTransform: 'uppercase', letterSpacing: 0.5 },
  summaryDivider: { width: 1, height: 32 },
  leaderRow: { flexDirection: 'row', justifyContent: 'space-around', paddingVertical: 10, paddingHorizontal: 12, borderTopWidth: 1 },
  leaderText: { fontSize: 12, fontFamily: 'Inter_400Regular' },
});
