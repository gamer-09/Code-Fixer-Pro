...
const CATALOG: CatalogItem[] = [
  ...
  { sym: 'PYUSD-USD', name: 'PayPal USD', cat: 'Crypto' },
  { sym: 'FDUSD-USD', name: 'First Digital USD', cat: 'Crypto' },
  { sym: 'USDE29470-USD', name: 'Ethena USDe', cat: 'Crypto' },
  { sym: 'TAO22974-USD', name: 'Bittensor (TAO)', cat: 'Crypto' },
  { sym: 'HYPE32196-USD', name: 'Hyperliquid (HYPE)', cat: 'Crypto' },
  { sym: 'ONDO-USD', name: 'Ondo Finance (RWA)', cat: 'Crypto' },
  { sym: 'COMP5692-USD', name: 'Compound', cat: 'Crypto' },
  { sym: 'CRO-USD', name: 'Cronos', cat: 'Crypto' },
  { sym: 'OKB-USD', name: 'OKB (OKX)', cat: 'Crypto' },
  { sym: 'OFFICIAL-TRUMP-USD', name: 'Official Trump', cat: 'Crypto' },
  // ── Indices ──
...
async function fetchQuoteDirect(sym: string): Promise<QuoteRow> {
  ...
  try {
    ...
    return {
      ...
      regularMarketVolume: (meta.regularMarketVolume as number | undefined) ?? 0,
...
async function fetchQuotes(symbols: string[]): Promise<Record<string, QuoteRow>> {
  ...
  // Process in batches to avoid URL length limits and server caps
        ...
        <FlatList
          ...
          renderItem={({ item: sym }) => (
            <WatchRow
              ...
              expanded={expandedSym === sym}
              onToggle={() => setExpandedSym(expandedSym === sym ? null : sym)}
            />
          )}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  expandCell: { width: '30%', minWidth: 80, marginBottom: 4 },
  ...
  expandHintText: { fontSize: 8, fontFamily: 'Inter_500Medium' },
